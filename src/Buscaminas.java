...
public class Buscaminas {
	...
	public void terminarPartida(int p) {
		...
		Puntuacion puntuacionFinal = null;
		switch (dificultadActual) {
		case 1:
			punt = PuntuacionesFacil.getPuntuacionesFacil();
			break;
		case 2:
			punt = PuntuacionesMedio.getPuntuacionesMedio();
			break;
		case 3:
			punt = PuntuacionesDificil.getPuntuacionesDificil();
			break;
		}
		int segundos = Cronometro.getCronometro().pararCronometro();
		int puntuacion = tablero.getNumDestapadas();
		puntuacionFinal = new Puntuacion(usuarioActual, puntuacion, segundos);
		punt.anadirPuntuacion(puntuacionFinal);

		// String ps =punt.mostrarPuntuaciones();
		if (puntuacionFinal != null)
			JOptionPane.showMessageDialog(null, puntuacionFinal, "Puntuaciones " + 1, JOptionPane.INFORMATION_MESSAGE,
					null);
		
		// Guardamos en BDD
		Connection con = BD.initBD("buscaminas.bd");
		Statement st = BD.usarCrearTablasBD(con);
		BD.insertarNuevaPuntuacion(st, puntuacionFinal);
		
		

		JugarDeNuevo frame = new JugarDeNuevo(usuarioActual);
